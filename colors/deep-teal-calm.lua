...
vim.g.colors_name = "deep-teal-calm"
...
local colors = {
  -- Backgrounds — low-chroma teal, four elevation tiers
  bg         = "#1B2A2E",   -- base
  bg_dark    = "#142124",   -- float panels, Telescope, NvimTree   (1.11:1 vs bg)
  bg_darker  = "#243437",   -- CursorLine, ColorColumn, QuickFix   (1.14:1 vs bg)
  bg_light   = "#2e3e42",   -- StatusLine, Pmenu, TabLine          (1.33:1 vs bg)
  bg_medium  = "#384a4f",   -- TabLineFill, BufferLine bg          (1.59:1 vs bg)

  -- Foregrounds — narrow band, hue-differentiated
  fg         = "#b7bdc0",   -- 7.80:1 — main text (cool gray)
  fg_soft    = "#b8bdb6",   -- 7.76:1 — identifiers (warm tint)
  fg_bright  = "#c0c4c7",   -- 8.44:1 — emphasis, headings

  -- Research-grounded 5-hue accent palette (warm split-complementary on H191)
  amber      = "#d2bc79",   -- 7.91:1  H45  — String, Constant, Boolean
  peach      = "#e3b18e",   -- 7.72:1  H25  — Number, Function, Method, @attribute
  coral      = "#ff9f95",   -- 7.51:1  H5   — Keyword, Operator, Error, Statement
  sage       = "#7ec8af",   -- 7.60:1  H160 — Type, Class, Module, Success, Hint
  lavender   = "#d8b2e4",   -- 8.05:1  H285 — Directory, Link, Markup heading

  -- Comments / dim — AA tier, recedes naturally
  comment    = "#8a9397",   -- 4.73:1 — Comments, LineNr, IndentMarker
  comment_dim = "#6f7a7e",  -- 3.36:1 — NonText, Whitespace, IndentBlank (below AA, intentional)

  amber_bright = "#d2bc79",  -- → amber  (was bright yellow)
  amber_medium = "#d2bc79",  -- → amber  (collapsed: String/Constant share weight)
  amber_deep   = "#e3b18e",  -- → peach  (Number/Function gets distinct warm hue)
  gold         = "#d2bc79",  -- → amber
  red_error    = "#ff9f95",  -- → coral  (Error gets bold attribute, not unique hue)
  red_keyword  = "#ff9f95",  -- → coral  (warm-red collapses 3 reds)
  red_operator = "#ff9f95",  -- → coral
  blue_type    = "#d8b2e4",  -- → lavender (warm-cool split-complement; pops on cool bg)
  green_success = "#7ec8af", -- → sage

  -- UI / structural
  selection  = "#2e3e42",   -- bg_light tint
  border     = "#2e3e42",   -- bg_light
  visual     = "#2d4146",   -- slightly tinted teal lift for visual selection
  cursor_line = "#243437",  -- bg_darker
  line_nr    = "#8a9397",   -- comment
  match_paren = "#d2bc79",  -- amber
  search     = "#e3b18e",   -- peach
